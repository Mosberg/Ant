...
export class AntUnit {
  ...
  findTask() {
    ...
    if (this.role === ROLE.WORKER) {
      ...
      const food =
        this.scene.findBestFoodForWorker?.(this) || this.scene.findNearestFood(this.x, this.y);
      if (food) {
        this.target = food;
  ...
  returnFood(dt) {
    ...
    if (!this.path || this.path.length <= 1) {
      this.moveToTile(deposit.tx, deposit.ty);
    }
  ...
  doDig(dt) {
    ...
    if (Phaser.Math.Distance.Between(this.x, this.y, tileCenterX, tileCenterY) < 14) {
      if (
        this.scene.map[this.target.ty][this.target.tx] === TILE.DIRT ||
        this.scene.map[this.target.ty][this.target.tx] === TILE.ROCK
      ) {
