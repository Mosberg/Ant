export class MobileSupport {
  ...
  bindTouchUnlock() {
    const resolveAudio = () => {
      if (this.options.audioManager) return this.options.audioManager;
      return this.game?.registry?.get?.("audio") || null;
    };
    ...
    const unlock = () => {
      ...
      try {
        ...
        // Ignore audio restrictions while unlocking.
      }
      const ctx = audio?.ctx;
  ...
  static attachSceneHelpers(scene) {
    ...
    scene.createMobileActionBar = function createMobileActionBar(buttons = []) {
      ...
      const pad = 10;
      const sidePad = pad + safe.left + safe.right;
      ...
      const columns = Math.max(
        1,
        Math.min(buttons.length, Math.floor((availableW + gap) / (minW + gap)))
      );
      const rows = Math.ceil(Math.max(buttons.length, 1) / columns);
      const btnW = Math.max(
        minW,
        Math.min(maxW, Math.floor((availableW - gap * (columns - 1)) / columns))
      );
      const btnH = rows > 1 ? 38 : 42;
      const startX = pad + safe.left + btnW / 2;
      const bottomY = h - safe.bottom - 34;
      const topY = bottomY - (rows - 1) * (btnH + rowGap);
      const barY = Math.max(64 + safe.top, topY);
      const barWidth = columns * btnW + (columns - 1) * gap;
      const barHeight = rows * btnH + (rows - 1) * rowGap;

      const group = this.add.container(0, 0).setScrollFactor(0).setDepth(999);
      const barBg = this.add
        .rectangle(
          pad + safe.left + barWidth / 2,
          barY + barHeight / 2,
          barWidth + 14,
          barHeight + 10,
          0x130f0d,
      ...
      buttons.forEach((btn, i) => {
        ...
        const y = barY + row * (btnH + rowGap);
        const rect = this.add
          .rectangle(x, y, btnW, btnH, 0x2d241d, 0.95)
          .setStrokeStyle(2, 0xd88c36, 1)
          .setInteractive({ useHandCursor: true });
        const label = this.add
          .text(x, y, btn.label, {
            fontSize: btnW < 52 ? "10px" : btnW < 72 ? "11px" : "13px",
            color: "#fff7ea",
            fontStyle: "bold",
            align: "center"
          })
