...
import { AntUnit } from "../entities/AntUnit.js";
...
export class AntManager {
  ...
  selectInRect(rect, append = false) {
    for (const ant of this.ants) {
      const hit = Phaser.Geom.Rectangle.Contains(rect, ant.x, ant.y);
      ant.selected = append ? ant.selected || hit : hit;
    }
  }

  selectClosestAt(x, y, maxDist = 18, append = false) {
    let best = null;
    let bestD = maxDist;
  ...
  selectByRole(role, append = false) {
    for (const ant of this.ants) {
      if (append) {
        ant.selected = ant.selected || ant.role === role;
      } else {
        ant.selected = ant.role === role;
      }
    }
  }
