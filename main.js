...
class AntUnit {
  ...
  returnFood() {
    ...
    if (distance(this, { x: deposit.tx * TILE_SIZE + 12, y: deposit.ty * TILE_SIZE + 12 }) < 14) {
  ...
  doFight(dt) {
    ...
    const targetTile = this.target.getTilePos ? this.target.getTilePos() : { tx: this.target.tx, ty: this.target.ty };
    ...
    if (distance(this, this.target) < 18 && this.attackCooldown <= 0) {
      const dmg = this.damage * (this.role === ROLE.SOLDIER ? this.scene.colonyModifiers.soldierPower : 1);
  ...
  doNursing(dt) {
    ...
    const c = brood.center();
    if (Phaser.Math.Distance.Between(this.x, this.y, c.tx * TILE_SIZE + 12, c.ty * TILE_SIZE + 12) < 18) {
...
class EnemyUnit {
  constructor(scene, tx, ty, type = ENEMY_TYPE.SPIDER) {
    ...
    this.healthBar = scene.add.rectangle(this.x - 10, this.y - 14, 20, 4, 0xde5a5a, 1).setOrigin(0, 0.5).setDepth(37);
  }
  ...
  update(dt) {
    ...
    if (this.type === ENEMY_TYPE.BEETLE) {
      ...
      if (room) {
        ...
        if (Phaser.Math.Distance.Between(this.x, this.y, roomCenter.x, roomCenter.y) < 16 && this.attackCooldown <= 0) {
...
class AntManager {
  ...
  assignRoleToSelection(role) {
    ...
    for (const ant of selected) {
      if (role === ROLE.SOLDIER && ant.role !== ROLE.SOLDIER && currentSoldiers >= this.scene.colonyModifiers.soldierCap) {
        continue;
      }
      ant.setRole(role);
    }
...
class MainMenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(w / 2, 140, "Ant Colony Manager", {
      fontFamily: "Arial",
      fontSize: "46px",
      color: "#f5e5c8",
      fontStyle: "bold"
    ...
    this.add.text(w / 2, 195, "Dig. Forage. Defend. Endure.", {
      fontSize: "20px",
      color: "#ceb48b"
    }).setOrigin(0.5);
    ...
    this.helpText = this.add.text(this.helpPanel.x - 340, this.helpPanel.y - 150,
`How to play
...
Space = 2x game speed`, {
  ...
  createButton(x, y, width, height, label, onClick) {
    const bg = this.add.rectangle(x, y, width, height, 0x3b2a1f, 1)
  ...
  createPanel(x, y, w, h, visible = true) {
    const panel = this.add.rectangle(x, y, w, h, 0x241b15, 0.97)
  ...
  buildOptionsPanel() {
    ...
    const title = this.add.text(this.optionsPanel.x, this.optionsPanel.y - 150, "Options", {
      fontSize: "28px",
      color: "#f6e6cb",
      fontStyle: "bold"
    }).setOrigin(0.5);
    ...
    const labels = [
      { key: "musicVolume", text: "Music Volume", type: "slider", min: 0, max: 1, step: 0.1 },
      { key: "sfxVolume", text: "SFX Volume", type: "slider", min: 0, max: 1, step: 0.1 },
      { key: "gameSpeed", text: "Game Speed", type: "choice", choices: [0.5, 1, 2] },
      { key: "graphicsDetail", text: "Graphics Detail", type: "choice", choices: [true, false] },
      { key: "difficulty", text: "Difficulty", type: "choice", choices: ["easy", "normal", "hard"] }
    ];

    this.optionsWidgets.push(title);

    labels.forEach(row => {
      const label = this.add.text(x, y, row.text, { fontSize: "22px", color: "#f3e4cf" });
      ...
      if (row.type === "choice") {
        ...
        row.choices.forEach(choice => {
          ...
          const btn = this.add.rectangle(bx, y + 14, 74, 34, chosen ? 0xd88c36 : 0x4b392c, 1)
            .setStrokeStyle(1, 0x9d7a50, 1)
            .setInteractive({ useHandCursor: true });
          const text = this.add.text(bx, y + 14, String(choice), {
            fontSize: "16px",
            color: "#fff7ea"
          }).setOrigin(0.5);
          btn.on("pointerdown", () => {
        ...
        for (let i = 0; i <= 10; i++) {
          const slot = this.add.rectangle(x + 220 + i * 24, y + 14, 18, 18, i / 10 <= val ? 0xd88c36 : 0x564232, 1)
...
class PauseScene extends Phaser.Scene {
  ...
  createBtn(x, y, label, cb) {
    const bg = this.add.rectangle(x, y, 220, 52, 0x3b2a1f, 1)
      .setStrokeStyle(2, 0xd88c36, 1)
      .setInteractive({ useHandCursor: true });
    const txt = this.add.text(x, y, label, {
      fontSize: "24px",
...
class UIScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.foodText = this.add.text(18, 10, "", { fontSize: "18px", color: "#f5ead6", fontStyle: "bold" }).setDepth(501);
    this.popText = this.add.text(220, 10, "", { fontSize: "18px", color: "#f5ead6", fontStyle: "bold" }).setDepth(501);
    this.moraleText = this.add.text(450, 10, "", { fontSize: "18px", color: "#f5ead6", fontStyle: "bold" }).setDepth(501);
    this.timeText = this.add.text(660, 10, "", { fontSize: "18px", color: "#f5ead6", fontStyle: "bold" }).setDepth(501);
    this.waveText = this.add.text(860, 10, "", { fontSize: "18px", color: "#f5ead6", fontStyle: "bold" }).setDepth(501);

    this.helpBox = this.add.rectangle(1110, 24, 620, 48, 0x33261d, 0.9).setOrigin(0.5, 0.5).setDepth(500);
    this.helpText = this.add.text(860, 10, "", { fontSize: "15px", color: "#dec8a3" }).setDepth(501);

    this.bottomPanel = this.add.rectangle(240, this.scale.height - 90, 470, 170, 0x1f1813, 0.94)
      .setStrokeStyle(2, 0x624d39, 1)
      .setDepth(500);

    this.roleLabel = this.add.text(26, this.scale.height - 162, "Assign Roles", {
      fontSize: "18px", color: "#f5ead6", fontStyle: "bold"
    }).setDepth(501);

    this.buildLabel = this.add.text(26, this.scale.height - 96, "Build Rooms", {
      fontSize: "18px", color: "#f5ead6", fontStyle: "bold"
    }).setDepth(501);

    this.debugText = this.add.text(this.scale.width - 250, 58, "", {
      fontSize: "14px",
      color: "#d8f2d0",
      backgroundColor: "#172012cc",
      padding: { x: 8, y: 6 }
    }).setDepth(520).setVisible(false);

    this.messageText = this.add.text(this.scale.width / 2, 66, "", {
      fontSize: "20px",
      color: "#fff4d8",
      fontStyle: "bold",
      backgroundColor: "#3a2c18cc",
      padding: { x: 12, y: 6 }
    }).setOrigin(0.5, 0).setDepth(510).setVisible(false);
  ...
  createBuildButtons() {
    ...
    let x = 120;
    rooms.forEach(room => {
      const btn = this.makeButton(x, this.scale.height - 66, 86, 36, ROOM_TYPES[room]?.label || "Utility", () => {
  ...
  createUtilityButtons() {
    this.makeButton(this.scale.width - 300, this.scale.height - 74, 90, 40, "Pause", () => this.togglePause());
    ...
    this.makeButton(this.scale.width - 100, this.scale.height - 74, 90, 40, "Help", () => this.toggleTutorial());
  }

  makeButton(x, y, w, h, label, cb) {
    const bg = this.add.rectangle(x, y, w, h, 0x4b392c, 1)
  ...
  update(_, dtMs) {
    ...
    this.helpText.setText(
      `Priority: ${g.globalPriority.toUpperCase()} | Build: ${(g.currentBuildMode || "none").toUpperCase()} | ` +
...
class GameScene extends Phaser.Scene {
  ...
  createHelpOverlay() {
    const box = this.add.rectangle(720, 440, 760, 300, 0x000000, 0.8)
      .setStrokeStyle(2, 0xb3844f, 1);
    const txt = this.add.text(390, 310,
`Tutorial
...
- Survive escalating waves; if the queen dies or morale reaches zero, the colony falls.`, {
  ...
  drawBuildPreview(worldX, worldY) {
    ...
    this.buildPreview.fillRect(tx * TILE_SIZE, ty * TILE_SIZE, def.size.w * TILE_SIZE, def.size.h * TILE_SIZE);
    this.buildPreview.strokeRect(tx * TILE_SIZE, ty * TILE_SIZE, def.size.w * TILE_SIZE, def.size.h * TILE_SIZE);
  }
  ...
  drawMinimap() {
    ...
    for (let y = 0; y < MAP_HEIGHT; y++) {
      for (let x = 0; x < MAP_WIDTH; x++) {
        let color = 0x000000;
        switch (this.map[y][x]) {
          case TILE.SURFACE: color = 0x5e8c3b; break;
          case TILE.DIRT: color = 0x5b3e29; break;
          case TILE.TUNNEL: color = 0x3a281b; break;
          case TILE.ROCK: color = 0x515151; break;
          case TILE.ROOM: color = this.getRoomColor(this.roomTypeMap[y][x]); break;
          case TILE.ENTRANCE: color = 0xd8bf8a; break;
        }
  ...
  getRoomColor(type) {
    switch (type) {
      case ROOM_KIND.QUEEN: return 0x9a6cc7;
      case ROOM_KIND.BROOD: return 0xd6c18a;
      case ROOM_KIND.STORAGE: return 0x8f6a3b;
      case ROOM_KIND.BARRACKS: return 0xa04747;
      case ROOM_KIND.UTILITY: return 0x5a7d44;
      default: return 0x6d533a;
    }
  }
  ...
  triggerLose(reason) {
    this.scene.pause();
    this.scene.pause("UIScene");
    this.add.rectangle(this.scale.width / 2, this.scale.height / 2, this.scale.width, this.scale.height, 0x000000, 0.72).setDepth(600);
    this.add.text(this.scale.width / 2, this.scale.height / 2 - 40, "Colony Lost", {
      fontSize: "44px",
      color: "#f0d7d7",
      fontStyle: "bold"
  ...
  triggerWin() {
    ...
    this.scene.pause("UIScene");
    this.add.rectangle(this.scale.width / 2, this.scale.height / 2, this.scale.width, this.scale.height, 0x000000, 0.72).setDepth(600);
    this.add.text(this.scale.width / 2, this.scale.height / 2 - 40, "Colony Endures", {
      fontSize: "44px",
      color: "#e6f0cb",
      fontStyle: "bold"
    }).setOrigin(0.5).setDepth(601);
    this.add.text(this.scale.width / 2, this.scale.height / 2 + 20, "You survived the final assault.", {
      fontSize: "24px",
      color: "#f2dfc4"
    }).setOrigin(0.5).setDepth(601);
  }
  ...
  update(_, dtMs) {
    ...
    if (Math.random() < 0.004 * DIFFICULTY_SETTINGS[this.settings.get("difficulty")].foodSpawnRate) {
