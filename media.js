/* Mobile support helpers for Ant Colony Manager
   - Adds robust touch and viewport handling
   - Keeps Phaser input unified through pointer events
   - Exposes optional mobile UI helpers for your scenes
*/
...
export class MobileSupport {
  ...
  bindFullscreenOnFirstTap() {
    ...
    const tryFullscreen = () => {
      ...
      if (typeof request === 'function' && !document.fullscreenElement) {
        try { request.call(el); } catch (_) {}
      }
    };

    document.addEventListener('pointerdown', tryFullscreen, { passive: true, once: true });
  }
  ...
  static attachSceneHelpers(scene) {
    ...
    scene.createMobileActionBar = function createMobileActionBar(buttons = []) {
      ...
      const gap = 10;
      const btnW = Math.min(110, Math.floor((w - pad * 2 - gap * (buttons.length - 1)) / Math.max(buttons.length, 1)));
      const btnH = 42;
      ...
      buttons.forEach((btn, i) => {
        ...
        const rect = this.add.rectangle(x, barY, btnW, btnH, 0x2d241d, 0.92)
          .setStrokeStyle(2, 0xd88c36, 1)
          .setInteractive({ useHandCursor: true });
        const label = this.add.text(x, barY, btn.label, {
          fontSize: '14px',
          color: '#fff7ea',
          fontStyle: 'bold'
        }).setOrigin(0.5);
